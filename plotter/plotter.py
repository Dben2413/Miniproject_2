...
def callback(ch, method, properties, body):
    ...
    if filename not in all_sample_data:
        all_sample_data[filename] = []
    all_sample_data[filename].append(data)
    print(data)
    last_message_time = time.time()


for method, properties, body in channel.consume(OUTPUT_QUEUE, inactivity_timeout=2):
...
for filename in all_sample_data:
    for i in all_sample_data[filename]:
